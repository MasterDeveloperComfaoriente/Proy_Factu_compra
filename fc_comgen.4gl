...
FUNCTION manimp()
 DEFINE mll char(7)
 DEFINE mimprima CHAR(500)
 DEFINE ubicacion1, ubicacion2 CHAR(150)
 DEFINE tp      RECORD
  a                CHAR(1),
 ...
 LET ubicacion1 = fgl_getenv("HOME"), "/rep1"
 LET ubicacion1=ubicacion1 clipped
 LET ubicacion2 = fgl_getenv("HOME"), "/lin"
 ...
 IF NOT maxnum THEN
  CALL FGL_WINMESSAGE( "Administrador", "NO HAY REGISTROS PARA VISUALIZAR", "information")
  LET tp.nombre = NULL
  DROP TABLE gentem
  RETURN
 END IF
 DISPLAY "" AT 1,10
 DISPLAY "" AT 2,1
 LET mimprima = "wc -l ", ubicacion clipped, " > ", ubicacion2 CLIPPED
 DISPLAY mimprima
 RUN mimprima
 LOAD FROM ubicacion2 INSERT INTO gentem
 LET mimprima = "rm ", ubicacion2 clipped
 RUN mimprima
 ...
 IF mdeflin <= 0 THEN
  CALL FGL_WINMESSAGE( "Administrador", "IMPRESION VACIA", "information")
  DROP TABLE gentem
  RETURN
 END IF
 IF mdeflin IS NULL THEN
  CALL FGL_WINMESSAGE( "Administrador", "IMPRESION VACIA", "information")
  DROP TABLE gentem
  RETURN
 END IF
 ...
 LABEL entrada:
 INPUT BY NAME o WITHOUT DEFAULTS
  AFTER FIELD o
   IF INFIELD(o) THEN
    CASE
     WHEN o="I"
     ...
     WHEN o="P"
      ...
      LET mimprima="awk 'FNR==",((mdefini*mdeftam)-mdeftam)+1 USING "<<<<<<<",
                   ...
                   " {print $0}' ", ubicacion clipped, " > ", ubicacion1 clipped
      RUN mimprima
      --LET mimprima = "less -S ", ubicacion1
      --RUN mimprima
      CALL showfile(ubicacion1)
      LET mimprima = "rm ", ubicacion1
      RUN mimprima
     WHEN o="A"
      LET o=null
      DISPLAY BY NAME o
     ...
     WHEN o="S"
      ...
      LET mimprima="awk 'FNR==",((mdefini*mdeftam)-mdeftam)+1 USING "<<<<<<<",
                   ...
                   " {print $0}' ", ubicacion clipped, " > ", ubicacion1 clipped
      RUN mimprima
      LET mimprima = "lp -d ",mdefimp clipped," -n ",mdefcop,
       " -o ",mdeflet clipped, " ", ubicacion1 clipped
      RUN mimprima
      LET mimprima = "rm ", ubicacion1 clipped
      RUN mimprima
      EXIT INPUT
     WHEN o="N"
      LET o=null
      DISPLAY BY NAME o
      LET mimprima = "rm ", ubicacion clipped
      RUN mimprima
      EXIT INPUT
   END CASE
   GOTO entrada
