...
FUNCTION envio_documento(tipodoc, prefijo,numfactu)
  DEFINE tipodoc, mtipdoc   LIKE    fe_factura_m.tipodocumento
  DEFINE prefijo            LIKE    fe_factura_m.prefijo
  DEFINE numfactu           LIKE    fe_factura_m.numfac 
  DEFINE mdoc               LIKE    fe_factura_m.documento
 ...
 OPEN WINDOW w_vista WITH FORM "Vista" DIALOG ATTRIBUTES(UNBUFFERED)
  ...
  BEFORE  DIALOG
   ...
   CALL cbtipd.clear()
   CALL cbtipd.addItem("1","FACTURA DE VENTA")
   CALL cbtipd.addItem("2","NOTA CREDITO")
   CALL cbtipd.addItem("3","NOTA DEBITO")
   CALL cbtipd.addItem("4","FACTURA DE EXPORTACION")
   CALL cbtipd.addItem("5","FACTURA CONTINGENCIA")
   CALL cbtipd.addItem("8","FACTURA SECTOR SALUD") 
   CASE
    WHEN tipodoc = "1" OR tipodoc = "4" OR tipodoc = "5"
  ...
  LET ls_salida = ls_salida,"\n+++++++++++++++++++++Consulta Método Respuesta Envio +++++++++++++++++++++"
   ...
   CALL Enlace_Doc_bbl.f_RespuestaEnvio_Recupera()
    ...
    LET ls_salida = ls_salida,"\nRespuesta Proceso: "
    LET ls_salida = ls_salida, estadoProceso CLIPPED, "-",  descripcionProceso
    --IF cufe IS NOT NULL THEN
     ...
     ELSE
       ...
       =  (cufe, fechaFactura,fechaRespuesta,
        fechaExpedicion,estadoProceso)
       WHERE tpdocumento = tipodoc
        AND prefijo = prefijo
        AND numfac = numfactu
   ...
   IF tipodoc = "1" THEN
    LET mtime=TIME
    CASE 
     WHEN (estadoProceso = "1" OR estadoProceso = "2" 
      OR estadoProceso = "3")   AND cufe IS NOT NULL 
       UPDATE fe_factura_m
        SET cufe = cufe, fecha_factura = fechaFactura, estado ="S", fecest = fechaRespuesta,
         codest = estadoProceso, hora = mtime     
        WHERE fe_factura_m.prefijo = prefijo
       AND fe_factura_m.numfac = numfactu
-----------------hospedaje---------------------
     { INITIALIZE mhos_rel_fact.* TO NULL
       SELECT * INTO mhos_rel_fact.* FROM hos_rel_fact
       WHERE hos_rel_fact.prefijo=prefijo and hos_rel_fact.documento=mdoc

       message "documento reserva: ",mdoc
       UPDATE hos_rel_fact
        SET numfac = numfactu    
        WHERE hos_rel_fact.prefijo = prefijo
       AND hos_rel_fact.documento = mdoc

       UPDATE hos_reserva
        SET estado ="O", dc=numfactu     
        WHERE hos_reserva.codigores  = mhos_rel_fact.codigores}

-----------------------------------------------------       
      WHEN estadoProceso= "24"  -- CONTINGENCIA.
       UPDATE fe_factura_m
    ...
    END CASE
   ELSE   
       if (estadoProceso = "1" OR estadoProceso = "2" 
      OR estadoProceso = "3")  AND cufe IS NOT NULL THEN 
        UPDATE fe_nota_m
         SET cude = cufe, fecha_nota = fechaFactura, estado ="S", fecest = fechaRespuesta,
          codest = estadoProceso, hora = mtime     
         WHERE fe_nota_m.tipo = prefijo
         AND fe_nota_m.numnota = numfactu
       END IF     
    END IF    
END FUNCTION
...
FUNCTION descarga_documento_masivo(tipodoc, prefijo, numfac)
  DEFINE tipodoc CHAR(1)
  DEFINE prefijo CHAR(4)
  DEFINE numfac  INTEGER
  ...
  LET newnamefile = fgl_getenv("HOME"),"/",mprefijo CLIPPED, "/",
       mprefijo CLIPPED, mnumfac USING "&&&&&&&"
  --LET ruta_descarga = "C:\\FacturasEscenarios\\Archivos\\"
  LET ruta_descarga = "/home/Facturas_Colegio/"
  EXECUTE IMMEDIATE "set encryption password ""0r13nt3"""
   select fe_dispapeles_acceso.idEmpresa
       ,""
       ,fe_dispapeles_acceso.usuario
...
FUNCTION consulta_estado_documento(tipodoc, prefijo, numfac)
  ...
  --Recupera el resultado de la consulta del Estado de la factura
    ...
    LET ls_salida = ls_salida,"\ncodigoQR      = ",codigoQr
    LET ls_salida = ls_salida,"\EstadoProceso       = ",estadoProceso
    ...
    IF fechaRespuesta IS NOT NULL AND numfactura IS NOT NULL THEN
      ...
      SELECT COUNT(*) INTO cnt 
       FROM fe_estados_fac
       WHERE fe_estados_fac.prefijo = prefijo
       AND fe_estados_fac.numfac = numfac
       AND fe_estados_fac.tpdocumento = tipodoc
      IF cnt IS NULL THEN LET cnt= 0 END IF
      IF cnt = 0 THEN 
      ...
      ELSE    
       ...
       SET (cufe, codult_estdisp, des_ult_estdisp,fecres_ultestdisp, cod_ult_estdian, des_ult_estdian, fecres_ultestdian, cod_ult_estmail,
        ...
        = ( cufe, codigoUltimoEstadoDispapeles,
         ...
         codigoUltimoEstadoAdquirente, descripcionUltimoEstadoAdquirente, fechaRespuestaUltimoEstadoAdquirente,  
         estadoProceso, fechaRespuesta, fechaFactura)
        WHERE fe_estados_fac.tpdocumento = tipodoc
         AND  fe_estados_fac.prefijo = prefijo
         AND  fe_estados_fac.numfac = numfac
      END if  
      IF estadoProceso= "1"  AND cufe IS NOT null THEN   -- EXITOSO
         CASE 
          WHEN tipodoc = "1"   
           UPDATE fe_factura_m
           SET  cufe = cufe, estado ="P", fecest = fechaRespuesta,  codest = estadoProceso    
           WHERE fe_factura_m.prefijo = prefijo
           AND fe_factura_m.numfac = numfac
           AND fe_factura_m.estado <> "N"
          WHEN tipodoc = "2" OR tipodoc ="3"   
           UPDATE fe_nota_m
            SET estado ="P", fecest = fechaRespuesta, codest = estadoProceso    
            WHERE fe_nota_m.tipo = prefijo
           AND fe_nota_m.numnota = numfac
           AND fe_nota_m.estado <> "N"
        END CASE
      END IF
...
FUNCTION envio_documento_fact(tipodoc, prefijo, numfactu)
  ...
  SELECT COUNT(*) INTO cantidadLineas
    FROM fe_factura_d
     WHERE trim(fe_factura_d.prefijo) = prefijo
  ...
  SELECT * INTO mfc_terceros.*
    FROM fe_terceros
     WHERE fe_terceros.nit = mfc_factura_m.nit
  INITIALIZE mfc_terobligacion.* TO NULL
  LET tipoobligacion=""
  DECLARE cur_obli_adq CURSOR FOR
  SELECT * 
   FROM fe_terobligacion
   WHERE fe_terobligacion.nit =  mfc_terceros.nit
   AND codigo_oblig IN ("O-13", "O-15", "O-23", "0-47")  -- Unicos validos ver. 1.8 fact res 13/2021
  ...
  IF mfc_terceros.medio_recep = "3" AND mfc_terceros.nit IS NOT NULL THEN
    LET envioPorEmailPlataforma = "PLATAFORMA"
    LET nitProveedorTecnologico = mfc_terceros.nit
  END IF  
  LET mhoy = CURRENT YEAR TO MINUTE  
  LET codigoPlantillaPdf = ""
  ...
  LET lineaResDian2= ""
   ...
   SELECT * INTO mfc_prefijos.*
    FROM fe_prefijos
     WHERE fe_prefijos.prefijo = mfc_factura_m.prefijo
     LET codigoPlantillaPdf = "14"
   ...
   {CALL Enlace_Doc_bbl.f_Adquirente_Agrega(
       "",        --barioLocalidad
       mfc_terceros.zona CLIPPED,   -- ciudad (nuevo campo)  
       "",        --codigoCIUU (Actividad economica)
       mfc_terceros.zona CLIPPED, 
       "",        --codigoPostal
       departamento,
       mfe_ciudades.nombreciu CLIPPED,    
       mfc_terceros.digver,
       mfc_terceros.direccion clipped,
       mfc_terceros.email CLIPPED,
       envioPorEmailPlataforma,
       "",        --matriculaMercantil,
       nitProveedorTecnologico, 
       nombreCompleto CLIPPED,
       mfe_deptos.nombredep CLIPPED,
       mfc_terceros.nit CLIPPED, 
       mfc_terceros.pais CLIPPED,
       mfe_paises.nombrepais CLIPPED,
       mfe_regimen.codreg,
       mfc_terceros.telefono CLIPPED,
       mfc_terceros.tipid,
       mfc_terceros.tipo_persona,
       tipoobligacion  CLIPPED
       )}
 -- LLAMADO DETALLE DEL DOCUMENTO
   ...
   LET Subtotal = 0
    ...
    SELECT * FROM fe_factura_d
     WHERE trim(fe_factura_d.prefijo) = mfc_factura_m.prefijo
     AND fe_factura_d.documento = mfc_factura_m.documento  
    ORDER BY fe_factura_d.codigo
    FOREACH Cur_DetDocto INTO mfc_factura_d.*
      LET posicion = posicion + 1
      LET TotalSub = TotalSub + (mfc_factura_d.subsi * mfc_factura_d.cantidad)
      LET TotalBen = TotalBen+ (mfc_factura_d.valorbene * mfc_factura_d.cantidad)
      LET descuento = (mfc_factura_d.subsi + mfc_factura_d.valorbene) * mfc_factura_d.cantidad
      ...
      SELECT * INTO mfc_servicios.* 
        FROM fe_servicios
         WHERE fe_servicios.codigo = mfc_factura_d.codigo
      LET nombreProducto = mfc_servicios.descripcion CLIPPED 
      IF mfc_factura_d.subcodigo IS NOT NULL AND mfc_factura_d.subcodigo <> "0" THEN
        INITIALIZE mfe_sub_servicios.* TO NULL
        SELECT * INTO mfe_sub_servicios.* 
        FROM fe_sub_servicios
         WHERE fe_sub_servicios.codigo = mfc_factura_d.subcodigo
        LET nombreProducto = nombreProducto CLIPPED, ' ', mfe_sub_servicios.descripcion CLIPPED
      END IF
      LET muestracomercial= 0
      IF mfc_factura_d.total_pagar = 0 THEN
        LET muestracomercial = "2"     -- SE INDICA QUE ES REGALO CUANDO ES TOTALMENTE SUBSIDIADO
      END IF   
      LET descuentoUni = (mfc_factura_d.subsi + mfc_factura_d.valorbene) --Descuento unitario rep. grafica
      CALL Enlace_Doc_bbl.f_DetalleDocumento_Agrega(
        ...
        "",             --seriales,
        0,             --tamanio,
        mfc_servicios.tpimpuesto,
        "999",            --tipocodigoproducto,
        mfc_servicios.coduni  CLIPPED,
   ...
   -- LLAMADO A AGREGAR LISTA DE IMPUESTOS
     ...
     IF mfc_factura_d.iva > 0  THEN 
      ...
      IF descuento>0 THEN
         LET porcentajeDescuento= 100*descuento/(mfc_factura_d.valoruni*mfc_factura_d.cantidad)  -- exigido DIAN
   ...
   END FOREACH  
    ...
    INITIALIZE mfe_factura_imp.* TO NULL
    DECLARE cur_impglo CURSOR FOR  
    SELECT * FROM fe_factura_imp
     WHERE trim(fe_factura_imp.prefijo) = mfc_factura_m.prefijo
     AND fe_factura_imp.documento = mfc_factura_m.documento  
    FOREACH cur_impglo INTO mfe_factura_imp.*
       CALL Enlace_Doc_bbl.f_Impuesto_Agrega(
          mfe_factura_imp.base,
          mfe_factura_imp.codimp,
          mfe_factura_imp.autoret,
          mfe_factura_imp.porcentaje,
          mfe_factura_imp.valor
          )
       CASE 
        WHEN mfe_factura_imp.codimp = "01"  
          LET TotalIva = TotalIva + mfe_factura_imp.valor 
        WHEN mfe_factura_imp.codimp = "02"  
          LET TotalImpc = TotalImpc + mfe_factura_imp.valor
       END CASE  
    END FOREACH
  ...
  END IF
   INITIALIZE mfe_factura_tot.* TO NULL
    SELECT * INTO mfe_factura_tot.* 
     FROM fe_factura_tot
     WHERE trim(fe_factura_tot.prefijo) = mfc_factura_m.prefijo
     AND fe_factura_tot.documento = mfc_factura_m.documento  
    CALL Enlace_Doc_bbl.f_Pagos_Agrega(
      mfe_empresa.moneda,       --codigoMonedaCambio
      NULL,         --  mfe_factura_tot.fecha_trm,
      fechaVencimiento,  
      mfe_factura_tot.moneda, 
      mfe_factura_tot.total_anticipos,
      periododepago,
      mfc_factura_m.forma_pago,
      mfe_factura_tot.total_cargos,
      0, -- mfe_factura_tot.total_descuentos,
      mfe_factura_tot.baseconimpu,
      mfe_factura_tot.baseimponible,
      mfe_factura_tot.total_factura,
      mfe_factura_tot.importebruto,
      mfe_factura_tot.trm, 
      NULL,          --trm_alterna,
      subtotalfac,
   ...
   --lista de anticipos de la factura 
     IF mfe_factura_tot.total_anticipos > 0 THEN
       DECLARE  cic_anti CURSOR for
        SELECT *  FROM fe_factura_anti
         WHERE fe_factura_anti.prefijo = mfc_factura_m.prefijo
           AND fe_factura_anti.documento = mfc_factura_m.documento
       FOREACH cic_anti INTO mfe_factura_anti.*
         CALL Enlace_Doc_bbl.f_Anticipo_Agrega(
         mfe_factura_anti.valor,
         mfe_factura_anti.doccru,
         ""
         )
       END FOREACH 
     END IF 
   -- LLAMADOS PARA AGREGAR CAMPOS ADICIONALES EN EL DISEÑO DE LA FACTURA.
     LET mvalche = mfe_factura_tot.total_factura
     CALL letras()
     LET lineaMonto1 = mletras1[1,69]
     LET lineaMonto2 = mletras2[1,70]
     CALL Enlace_Doc_bbl.f_CampoAdicional_Agrega(NULL,"Regimen",1,1,mfe_empresa.regimen CLIPPED,"")
     ...
     CALL Enlace_Doc_bbl.f_CampoAdicional_Agrega(NULL,"LeyendaEncab1",1,1,mfe_empresa.leyenda_enc CLIPPED,"")
     ...
     CALL Enlace_Doc_bbl.f_CampoAdicional_Agrega(NULL,"PiePagina2",1,1,mfe_empresa.piepag_2,"")
     CALL Enlace_Doc_bbl.f_CampoAdicional_Agrega(NULL,"PiePagina3",1,1,mfe_empresa.piepag_3,"")
...
FUNCTION enviar_factura_dian()
 ...
 DEFINE mtipdoc     integer
  ...
  IF mfc_factura_m.numfac IS NULL THEN
    ...
    RETURN
  END IF
  -- validar si la factura corresponde a salud -- cambio de tipodocumento res 13/2021
  LET mtipdoc = 0
  LET cnt = 0
  SELECT COUNT(*) INTO cnt 
  FROM fe_servicios_prefijos
   WHERE  fe_servicios_prefijos.codservicio = "5"
   AND  fe_servicios_prefijos.prefijo = mpref
  IF cnt = 1 THEN
   LET mtipdoc = 8
  ELSE
   LET mtipdoc = 1
  ...
  --CALL envio_documento(mtipdoc, mfc_factura_m.prefijo, mfc_factura_m.numfac)
END FUNCTION

FUNCTION enviar_nota_dian()
 DEFINE mtiponota       CHAR(2)     
 DEFINE mnumnota        LIKE   fe_nota_m.numnota
  LET mtiponota = NULL 
  LET mnumnota = 0
  ...
  SELECT * INTO mfe_nota_m.*
    FROM fe_nota_m
  WHERE tipo = mtiponota
   AND numnota = mnumnota
  IF mfe_nota_m.numnota IS NULL THEN
    CALL FGL_WINMESSAGE( "Mensaje de error ", "El numero de la nota no fue encontrada","stop")
    RETURN
  END IF
  IF mfe_nota_m.estado = "B" THEN
    CALL FGL_WINMESSAGE( "Información", "La Nota aun se encuentra en estado Borrador","information")
    RETURN
  END IF
  CALL act_totales_nota( mfe_nota_m.tipo, mfe_nota_m.documento)
  IF mtiponota ="NC" THEN
     CALL envio_documento("2",mtiponota, mfe_nota_m.numnota)
  ELSE
     CALL envio_documento("3",mtiponota, mfe_nota_m.numnota)
  END IF   
END FUNCTION
...
FUNCTION consulta_estados_factu_masivo()
 DEFINE mpref       LIKE   fe_factura_m.prefijo
 DEFINE  mnumfactu, mnumfactu1, mnumfactu2   LIKE   fe_factura_m.numfac
  LET mpref = NULL 
  LET mnumfactu1 = 0
  LET mnumfactu2 = 0
  PROMPT "Prefijo  ===> : "    FOR mpref
  ...
  IF mpref IS NULL THEN 
    RETURN
  END IF
  PROMPT "Numero de factura (Inicial)  :  "   FOR mnumfactu1
  IF mnumfactu1 IS NULL OR mnumfactu1 = 0 THEN
    RETURN
   END IF
  PROMPT "Numero de factura (Final)  :  "   FOR mnumfactu2
...
FUNCTION generar_pdf_factu_colegio()
 DEFINE mpref        LIKE   fe_factura_m.prefijo
 DEFINE mnumfactu    LIKE   fe_factura_m.numfac
 DEFINE mnumfactufin LIKE   fe_factura_m.numfac
 DEFINE bandera BOOLEAN 
  LET mpref = NULL 
  LET mnumfactu = 0
  PROMPT "Digite el prefjo:---------->" FOR mpref
  LET mpref =  upshift(mpref)
  IF mpref IS NULL THEN
    RETURN 
  END IF 
  PROMPT "Numero de factura inicial :  "   FOR mnumfactu
  IF mnumfactu IS NULL OR mnumfactu = 0 THEN
    RETURN
   END IF
  LET bandera = FALSE 
  WHILE bandera = FALSE
    PROMPT "Numero de factura final :  " FOR mnumfactufin
    IF mnumfactufin > mnumfactu THEN
        LET bandera = TRUE 
    ELSE 
        CALL fgl_winmessage("Administrador","EL NUMERO FINAL DEBE SER MAYOR AL INICIAL","stop")
    END IF 
  END WHILE 
  INITIALIZE mfc_factura_m.* TO NULL
  ...
  WHERE prefijo = mpref
  AND numfac BETWEEN  mnumfactu AND mnumfactufin
  FOREACH cur_facturas INTO mfc_factura_m.*
      IF mfc_factura_m.numfac IS NULL THEN
        CALL FGL_WINMESSAGE( "Mensaje de error ", "El numero de factura no fue encontrada","stop")
        RETURN
      END IF
      IF mfc_factura_m.estado = "B"  THEN
        ...
        RETURN
      END IF
      CALL descarga_documento_masivo("1",mfc_factura_m.prefijo, mfc_factura_m.numfac)
  END FOREACH 
END FUNCTION

FUNCTION generar_pdf_factu_colegio_fechas_curso()
 DEFINE mpref        LIKE   fe_factura_m.prefijo
 DEFINE fecha_inicial, fecha_final DATE
 DEFINE curso CHAR (3) 
 LET mpref="EDUC"

  CALL fec_cur() RETURNING fecha_inicial, fecha_final,curso
  INITIALIZE mfc_factura_m.* TO NULL
  DECLARE cur_facturas2 CURSOR FOR 
    select * from fe_factura_m,fe_factura_ter,c_alumno
    where fe_factura_m.documento=fe_factura_ter.documento
    and fe_factura_ter.cedula=c_alumno.documento
    and fe_factura_m.prefijo=fe_factura_ter.prefijo
    and codcurso=curso
    and fe_factura_m.fecha_factura between fecha_inicial and fecha_final
    and vigencia=year(fe_factura_m.fecha_factura)
    and fe_factura_m.prefijo=mpref
    and fe_factura_m.estado="P"
  FOREACH cur_facturas2 INTO mfc_factura_m.*
      IF mfc_factura_m.numfac IS NULL THEN
        CALL FGL_WINMESSAGE( "Mensaje de error ", "El numero de factura no fue encontrada","stop")
        RETURN
      END IF
...
FUNCTION fec_cur()
DEFINE fechaini, fechafin DATE,
curse CHAR (3)
OPEN WINDOW w_fecha_curso AT 04,6 WITH FORM "fec_cur"
INPUT fechaini,fechafin,curse FROM fechai,fechaf,curso
    
AFTER FIELD fechai
    IF fechaini IS NULL THEN
        CALL fgl_winmessage("Administrador","DIGITE UNA FECHA INICIAL EN FORMATO MM/DD/AAAA","stop")
        NEXT FIELD fechai
    END IF 
AFTER FIELD fechaf
    IF fechafin IS NULL THEN
        CALL fgl_winmessage("Administrador","DIGITE UNA FECHA FINAL EN FORMATO MM/DD/AAAA","stop")
        NEXT FIELD fechaf
    ELSE 
        IF fechafin < = fechaini THEN 
            CALL fgl_winmessage("Administrador","LA FECHA FINAL DEBE SER MAYOR A LA FECHA FINAL","stop")
            NEXT FIELD fechaf
        END IF 
    END IF
AFTER FIELD curso
    IF curse IS NULL THEN
        CALL fgl_winmessage("Administrador","SELECCIONE UNA CURSO","stop")
        NEXT FIELD curso
    END IF 
AFTER INPUT
  IF int_flag THEN
  ...
  END IF
 END INPUT
CLOSE WINDOW w_fecha_curso
LET curse=curse CLIPPED 
RETURN fechaini,fechafin,curse 
END FUNCTION 
...
FUNCTION consulta_estados_nota()
 ...
 DEFINE mnumnota        LIKE   fe_nota_m.numnota
  ...
  END IF
  IF mtiponota ="NC" THEN
    CALL consulta_estado_documento("2",mtiponota, mfe_nota_m.numnota)
  ELSE
    CALL consulta_estado_documento("3",mtiponota, mfe_nota_m.numnota)
  END IF  
END FUNCTION
...
DEFINE mtiponota       CHAR(2)     
 DEFINE mnumnota        LIKE   fe_nota_m.numnota
  ...
  END IF
  IF mtiponota ="NC" THEN
    CALL descarga_documento("2",mtiponota, mfe_nota_m.numnota)
  ELSE
    CALL descarga_documento("3",mtiponota, mfe_nota_m.numnota)
  END IF
END FUNCTION

FUNCTION envio_documento_nota(tipodoc, prefijo, numfactu)
  DEFINE tipodoc, mtipdoc   LIKE    fe_nota_m.tipodocumento
  DEFINE prefijo            LIKE    fe_nota_m.tipo
  DEFINE numfactu           LIKE    fe_nota_m.numnota
  DEFINE mdocumento         CHAR(7) 
  LET lineaNota1= ""
  LET lineaNota2= ""
  LET lineaNota3= ""
  ...
  SELECT * INTO mfe_nota_m.*
    FROM fe_nota_m
     WHERE fe_nota_m.tipo = prefijo
     AND fe_nota_m.numnota = numfactu
  LET mdocumento = mfe_nota_m.documento 
    LET ls_salida = ls_salida,"\n--------------------------Proceso Metodo Enviar Nota (Debito/Credito)---------------------------------"
    LET ls_salida = ls_salida,"\n+++++++++++++++++++++Inicia obtencion de datos+++++++++++++++++++++"
    CALL ui.Interface.refresh()
    LET ls_salida = ls_salida, "\nResultado del Encabezado Nota -  Documento Interno: ", mfc_factura_m.documento
  LET lineaNota1 = mfe_nota_m.nota1[1,70]
  LET lineaNota2 = mfe_nota_m.nota1[71,140]
  LET lineaNota3 = mfe_nota_m.nota1[141,210]
  LET lineaNota4 = mfe_nota_m.nota1[211,280]
  SELECT nombre INTO NombreAprobo 
    FROM gener02
   WHERE gener02.usuario = mfe_nota_m.usuario_apru
  SELECT nombre INTO NombreElaboro 
    FROM gener02
   WHERE gener02.usuario = mfe_nota_m.usuario_add
  LET cantidadLineas = 0  
  INITIALIZE mfc_factura_m.* TO NULL
  SELECT * INTO mfc_factura_m.*
   FROM fe_factura_m
  WHERE fe_factura_m.prefijo = mfe_nota_m.prefijo 
    AND fe_factura_m.numfac = mfe_nota_m.numfac 
  SELECT COUNT(*) INTO cantidadLineas
    FROM fe_nota_d
     WHERE trim(fe_nota_d.tipo) = prefijo
     AND fe_nota_d.documento = mdocumento
   CASE   
     WHEN mfe_nota_m.tipo = "NC"
       IF mfc_factura_m.cufe IS NOT NULL THEN 
         LET mfe_nota_m.tipoope = "20"
       ELSE
         LET mfe_nota_m.tipoope = "22"
       END IF   
    WHEN mfe_nota_m.tipo = "NC"
      IF mfc_factura_m.cufe IS NOT NULL THEN 
        LET mfe_nota_m.tipoope = "30"
      ELSE
        LET mfe_nota_m.tipoope = "32"
      END IF  
   END CASE   
...
CASE mfe_nota_m.tipo_nota
  ...
  END CASE  
   ...
   SELECT * INTO mfc_prefijos.*
    FROM fe_prefijos
     WHERE fe_prefijos.prefijo = mfe_nota_m.prefijo
     IF  mfc_prefijos.num_plantilla = 1 THEN
       IF mfe_nota_m.tipo = "NC" THEN
         LET codigoPlantillaPdf = 1     
       ELSE
         LET codigoPlantillaPdf = 3
       END IF
     END IF
     IF  mfc_prefijos.num_plantilla = 2 THEN
       IF mfe_nota_m.tipo = "NC" THEN
         LET codigoPlantillaPdf = 2     
       ELSE
         LET codigoPlantillaPdf = 4
       END IF
     END IF
   SELECT detzon INTO ciudadFacturador 
     FROM gener09
   ...
   CALL Enlace_Doc_bbl.f_CabezaDocumento_Agrega(
     ...
     "DAP",                 -- incoterm
     "",                    --  nombrevendedor
     mfe_nota_m.tipo,       -- prefijo   (para pruebas es NC o ND)
     "",                    --  Sucursal
     mfe_nota_m.tipoope,
     tipodoc,
     mfe_nota_m.tipo_nota,
     token,usuario,version,
     nombreImpresora,
     campoAdicional1,
     campoAdicional2,
   ...
   CALL Enlace_Doc_bbl.f_Adquirente_Agrega(
        "",        --barioLocalidad 1
       mfc_terceros.zona CLIPPED, -- Nombre ciudad duplicado 2
 ...
 -- CUFE SE ESTABA CAPTURANDO TRUNCADO (29/01/2020) 
   LET cufe = null
   INITIALIZE mfe_estados_fac.* TO NULL
   SELECT * INTO mfe_estados_fac.*
     FROM fe_estados_fac
      WHERE fe_estados_fac.prefijo = mfe_nota_m.prefijo
      AND fe_estados_fac.numfac = mfe_nota_m.numfac
    IF mfe_estados_fac.cufe IS NOT NULL THEN
      LET cufe = mfe_estados_fac.cufe
    END IF   
    IF mfe_nota_m.tipoope = "20" OR  mfe_nota_m.tipoope = "30" THEN   
      CALL Enlace_Doc_bbl.f_FacturaModificada_Agrega( 
        mfe_nota_m.numfac,                  --consecutivoFacturaModificada,
        cufe CLIPPED,                      --cufeFacturaModificada,
        mfc_factura_m.fecha_factura,        --fechaFacturaModificada,
        "",                                 --observacion,
        mfe_nota_m.prefijo CLIPPED,                 -- prefijoFacturaModificada,
 ...
 -- LLAMADO DETALLE DEL DOCUMENTO
   ...
   DECLARE Cur_DetNota CURSOR FOR
    SELECT * FROM fe_nota_d
     WHERE trim(fe_nota_d.tipo) = mfe_nota_m.tipo
     AND fe_nota_d.documento = mfe_nota_m.documento  
    ORDER BY fe_nota_d.codigo
    FOREACH Cur_DetNota INTO mfe_nota_d.*
      LET posicion = posicion + 1
      LET TotalSub = TotalSub + (mfe_nota_d.subsi * mfe_nota_d.cantidad)
      LET TotalBen = TotalBen+ (mfe_nota_d.valorbene * mfe_nota_d.cantidad)
      LET descuento = (mfe_nota_d.subsi + mfe_nota_d.valorbene) * mfe_nota_d.cantidad
      LET Subtotal = Subtotal + (mfe_nota_d.valoruni * mfe_nota_d.cantidad)
      INITIALIZE mfc_servicios.* TO NULL
      SELECT * INTO mfc_servicios.* 
        FROM fe_servicios
         WHERE fe_servicios.codigo = mfe_nota_d.codigo
      LET nombreProducto = mfc_servicios.descripcion CLIPPED 
      IF mfe_nota_d.subcodigo IS NOT NULL AND mfe_nota_d.subcodigo <> "0" THEN
        INITIALIZE mfe_sub_servicios.* TO NULL
        SELECT * INTO mfe_sub_servicios.* 
        FROM fe_sub_servicios
         WHERE fe_sub_servicios.codigo = mfe_nota_d.subcodigo
        LET nombreProducto = nombreProducto CLIPPED, ' ', mfe_sub_servicios.descripcion CLIPPED
      END IF
      LET muestracomercial= 0
      IF mfe_nota_d.total_pagar = 0 THEN
        LET muestracomercial = "2"     -- SE INDICA QUE ES REGALO CUANDO ES TOTALMENTE SUBSIDIADO
      END IF
      LET descuentoUni = (mfe_nota_d.subsi + mfe_nota_d.valorbene) --Descuento unitario rep. grafica
      CALL Enlace_Doc_bbl.f_DetalleDocumento_Agrega(
        "no",           --aplicaMandato,
        0,             -- CampoAdicional1
        0,             --campoAdicional2,
        ...
        nombreProducto,
        posicion,
        mfe_nota_d.base_imponible,
        mfe_nota_d.total_pagar,
        "",             --referencia,
        "",             --seriales,
        0,             --tamanio,
        mfc_servicios.tpimpuesto,
      ...
      -- LLAMADO A AGREGAR LISTA DE IMPUESTOS
      LET isAutoRetenido = 0
      IF mfe_nota_d.iva =  0  AND mfe_nota_d.impc =0 THEN 
          CALL Enlace_Doc_bbl.f_Impuesto_Detalle_Agrega(
          mfe_nota_d.base_imponible,          
          "01",                     --codigoImpuestoRetencion  -  IVA
          isAutoRetenido,
          mfc_servicios.iva,
          mfe_nota_d.total_iva)
      END IF
     IF mfe_nota_d.iva > 0  THEN 
          CALL Enlace_Doc_bbl.f_Impuesto_Detalle_Agrega(
          mfe_nota_d.base_imponible,          
          "01",                     --codigoImpuestoRetencion  -  IVA
          isAutoRetenido,
          ...
          mfe_nota_d.total_iva)
      END IF
      IF mfe_nota_d.impc > 0  THEN
          CALL Enlace_Doc_bbl.f_Impuesto_Detalle_Agrega(
          mfe_nota_d.base_imponible,          
          "02",                     --codigoImpuestoRetencion  -  IVA
          isAutoRetenido,
          mfc_servicios.impc,
          mfe_nota_d.total_impc) 
      END IF    
      -- LLAMADO A AGREGAR LISTA DE DESCUENTOS  ( EN NUESTRO CASOS SUBSIDIOS
      IF descuento>0 THEN
         LET porcentajeDescuento= 100*descuento/(mfe_nota_d.valoruni*mfe_nota_d.cantidad)  -- exigido DIAN
         CALL Enlace_Doc_bbl.f_Descuento_Detalle_Agrega(
         "11",                                  --codigoDescuento,
         "SUBSIDIO EN TARIFA Y/O ESPECIE",       --descuento_descripcion,
         descuento,
   ...
   END FOREACH  
    ...
    SELECT * FROM fe_nota_imp
     WHERE trim(fe_nota_imp.prefijo) = mfe_nota_m.tipo
     AND fe_nota_imp.documento = mfe_nota_m.documento  
    FOREACH cur_impglo_not INTO mfe_nota_imp.*
       CALL Enlace_Doc_bbl.f_Impuesto_Agrega(
          mfe_nota_imp.base,
          mfe_nota_imp.codimp,
          mfe_nota_imp.autoret,
          mfe_nota_imp.porcentaje,
          mfe_nota_imp.valor
          )
       CASE 
        WHEN mfe_nota_imp.codimp = "01"  
          LET TotalIva =  TotalIva +  mfe_nota_imp.valor
        WHEN mfe_nota_imp.codimp = "02"  
          LET TotalImpc = TotalImpc + mfe_nota_imp.valor
       END CASE  
    END FOREACH
  -- LLAMADO A TOTALES DE NOTA (PAGOS)
  LET fechaVencimiento = NULL
  LET periododepago = 0
  INITIALIZE mfe_nota_tot.* TO NULL
    SELECT * INTO mfe_nota_tot.* 
     FROM fe_nota_tot
     WHERE trim(fe_nota_tot.prefijo) = mfe_nota_m.tipo
     AND fe_nota_tot.documento = mfe_nota_m.documento  
    CALL Enlace_Doc_bbl.f_Pagos_Agrega(
      mfe_empresa.moneda,       --codigoMonedaCambio
      NULL,         --  mfe_factura_tot.fecha_trm,
      fechaVencimiento,  
      mfe_nota_tot.moneda, 
      mfe_nota_tot.total_anticipos,
      periododepago,
      "1",       ---mfc_factura_m.forma_pago,  1-> CONTADO
      mfe_nota_tot.total_cargos,
      0, -- mfe_factura_tot.total_descuentos,
      mfe_nota_tot.baseconimpu,
      mfe_nota_tot.baseimponible,
      mfe_nota_tot.totalnota,
      mfe_nota_tot.importebruto,
      mfe_nota_tot.trm, 
      NULL,     --trm_altern
      subtotalfac,
      valorEnLetrasSubTotal,
   ...
   -- LLAMADOS PARA AGREGAR CAMPOS ADICIONALES EN EL DISEÑO DE LA NOTA.
     LET mvalche = mfe_nota_tot.totalnota
     CALL letras()
     LET lineaMonto1 = mletras1[1,69]
     LET lineaMonto2 = mletras2[1,70]
    IF mfe_nota_m.tipoope = "20" OR  mfe_nota_m.tipoope = "30" THEN
         ...
         CASE 
          ...
          WHEN tipodoc = "2" OR tipodoc ="3"   
           UPDATE fe_nota_m
            ...
            WHERE fe_nota_m.tipo = prefijo
           AND fe_nota_m.numnota = numfac
           AND fe_nota_m.estado <> "N"  -- NO SE PUEDE ALTERAR 
        END CASE
      END IF
      IF codigoUltimoEstadoAdquirente = "3"  THEN     -- RECHAZADA CLIENTE
         CASE 
